...
func TestABCIResponsesSaveLoad1(t *testing.T) {
	...
	abciResponses.EndBlock = &abci.ResponseEndBlock{ValidatorUpdates: []abci.ValidatorUpdate{
		types.TM2PB.NewValidatorUpdate(gost512.GenPrivKeyWithInput(testSubject, testPassword).PubKey(), 10),
	}}
...
func genValSetWithPowers(powers []int64) *types.ValidatorSet {
	...
	for i := 0; i < size; i++ {
		...
		val := types.NewValidator(gost512.GenPrivKeyWithInput(testSubject, testSubject).PubKey(), powers[i])
		val.ProposerPriority = tmrand.Int64()
		vals[i] = val
	}
	valSet := types.NewValidatorSet(vals)
...
// see https://github.com/number571/tendermint/issues/2718
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	tearDown, _, state := setupTestCase(t)
	defer tearDown(t)
	val1VotingPower := int64(10)
	val1PubKey := gost512.GenPrivKeyWithInput(testSubject, testPassword).PubKey()
	...
	// add a validator
	val2PubKey := gost512.GenPrivKeyWithInput(testSubject, testPassword).PubKey()
...
func TestLargeGenesisValidator(t *testing.T) {
	...
	genesisPubKey := gost512.GenPrivKeyWithInput(testSubject, testPassword).PubKey()
	...
	// see: https://github.com/number571/tendermint/issues/2960
	firstAddedValPubKey := gost512.GenPrivKeyWithInput(testSubject, testPassword).PubKey()
	...
	for i := 0; i < 10; i++ {
		addedPubKey := gost512.GenPrivKeyWithInput(testSubject, testPassword).PubKey()
