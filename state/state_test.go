...
func TestABCIResponsesSaveLoad1(t *testing.T) {
	...
	abciResponses.EndBlock = &abci.ResponseEndBlock{ValidatorUpdates: []abci.ValidatorUpdate{
		types.TM2PB.NewValidatorUpdate(gost512.GenPrivKey().PubKey(), 10),
	}}
...
func genValSetWithPowers(powers []int64) *types.ValidatorSet {
	...
	for i := 0; i < size; i++ {
		...
		val := types.NewValidator(gost512.GenPrivKey().PubKey(), powers[i])
		val.ProposerPriority = tmrand.Int64()
		vals[i] = val
	}
	valSet := types.NewValidatorSet(vals)
...
func TestProposerPriorityDoesNotGetResetToZero(t *testing.T) {
	...
	val1PubKey := gost512.GenPrivKey().PubKey()
	...
	// add a validator
	val2PubKey := gost512.GenPrivKey().PubKey()
...
func TestLargeGenesisValidator(t *testing.T) {
	...
	genesisPubKey := gost512.GenPrivKey().PubKey()
	...
	firstAddedValPubKey := gost512.GenPrivKey().PubKey()
	...
	for i := 0; i < 10; i++ {
		addedPubKey := gost512.GenPrivKey().PubKey()
